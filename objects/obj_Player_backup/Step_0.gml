...
{
	if (place_meeting(
		x + _horSpeed.x	+ right_vector.x * sign(_horVelocity) * player_gravity_is_diagonal(),
		y + _horSpeed.y	+ right_vector.y * sign(_horVelocity) * player_gravity_is_diagonal(),
		obj_Block))
	{
		while (!place_meeting(
			x + right_vector.x * sign(_horVelocity) * (1 + player_gravity_is_diagonal()),
			y + right_vector.y * sign(_horVelocity) * (1 + player_gravity_is_diagonal()),
			obj_Block))
		{
			x += right_vector.x * sign(_horVelocity);
			y += right_vector.y * sign(_horVelocity);
		}
		
		/*move_contact_object(right_vector.mult(sign(_horVelocity)), _horVelocity, obj_Block);
	...
	{
		while (!place_meeting(
			x + down_vector.x * sign(grav_spd) * (1 + player_gravity_is_diagonal()),
			y + down_vector.y * sign(grav_spd) * (1 + player_gravity_is_diagonal()),
			obj_Block))
		{
			x += down_vector.x * sign(grav_spd);
			y += down_vector.y * sign(grav_spd);
		}
		grav_spd = 0;
	}
	else
